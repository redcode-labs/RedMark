...
We are trying to unify and enable the Proxmark to easily detect and select correct timings automaticly.
...
#define PRN_NA   sprintf(s  + strlen(s), _RED_("N/A") " | ");
    ...
    Dbprintf("---------------------------+-----+-----+-----+-----+-----+-----+------");
    Dbprintf("           mode            | g s | g w | w 0 | w 1 | g r | w 2 | w 3");
    Dbprintf("---------------------------+-----+-----+-----+-----+-----+-----+------");
...
void WriteTItag(uint32_t idhi, uint32_t idlo, uint16_t crc, bool ledcontrol) {
    ...
    // finish with 75ms programming time
...
void EM4xBruteforce(uint32_t start_pwd, uint32_t n, bool ledcontrol) {
    ...
    for (uint32_t pwd = start_pwd; pwd < 0xFFFFFFFF; pwd++) {
        ...
        //WaitUS(400);
        WaitUS(375);
        //DoPartialAcquisition(0, true, 350, 1000, ledcontrol);

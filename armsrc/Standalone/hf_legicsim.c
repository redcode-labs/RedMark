...
#define HF_LEGICSIM_DUMPFILE_SIM "legic_%02d.bin"
#define DUMP_SIZE 1024

static char lcur_dump_file[24] = {0};

static bool lfill_eml_from_file(char *dumpfile) {
    // check file exist
...
static bool lwrite_file_from_eml(char *dumpfile) {
    if (!exists_in_spiffs(dumpfile)) {
        Dbprintf(_RED_("Dump file %s not found!"), dumpfile);
        return false;
    }
...
void RunMod(void) {
    ...
    for (int i = 1;; i++) {
        ...
        sprintf(lcur_dump_file, HF_LEGICSIM_DUMPFILE_SIM, i);
        Dbprintf(_YELLOW_("[Slot: %d] Try to load dump file: %s"), i, lcur_dump_file);
        if (!lfill_eml_from_file(lcur_dump_file)) {
        ...
        if (!lwrite_file_from_eml(lcur_dump_file)) {

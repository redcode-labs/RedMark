...
static void save_dump_to_file(legic_card_select_t *p_card) {
    ...
    char *outfile = (char *)BigBuf_malloc(preferredNameLen + 4 + 1 + 10);
    if (outfile == NULL) {
        goto OUT;
    }

    sprintf(outfile, "%.*s%s", preferredNameLen, preferredName, ".bin");
    uint16_t num = 1;
    while (exists_in_spiffs(outfile)) {
        sprintf(outfile, "%.*s-%d%s", preferredNameLen, preferredName, num, ".bin");
        num++;
    }

    rdv40_spiffs_write(outfile, mem, p_card->cardsize, RDV40_SPIFFS_SAFETY_SAFE);

    Dbprintf("[=] saved card dump to flashmem::" _YELLOW_("%s"), outfile);
OUT:
    BigBuf_free_keep_EM();
#endif

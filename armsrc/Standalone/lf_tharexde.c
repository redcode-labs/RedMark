 ...
 * reading EM4x50 data. Each collected data set will be written/appendered to the
...
static void DownloadLogs(const char *logfile) {
    Dbprintf("");
...
static void appender(const char *filename, uint8_t *entry, size_t entry_len) {
...
static void save_pwds(uint32_t *pwdlist, size_t no_pwd) {
    if (no_pwd > 0) {
        ...
        for (int i = 0; i < no_pwd; i++) {
            ...
            appender(LF_EM4X50_LOGFILE_SIM, entry, strlen((char *)entry));
...
void RunMod(void) {
    ...
    for (;;) {
        ...
        if (state == STATE_SIM) {
            ...
            if (no_words > 0) {
                ...
                appender(LF_EM4X50_LOGFILE_COLLECT, entry, strlen((char *)entry));
            }
        }
    ...
    if (state == STATE_READO) {
        DownloadLogs(LF_EM4X50_LOGFILE_COLLECT);
    } else {
        // save and display passwords
        save_pwds(pwdlist, no_pwd);
        DownloadLogs(LF_EM4X50_LOGFILE_SIM);
    }

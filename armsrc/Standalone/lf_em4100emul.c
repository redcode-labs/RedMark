...
static uint64_t em4100emul_low[] = {0x565A1140BE, 0x365A398149, 0x5555555555, 0xFFFFFFFFFF, 0x5C008BB55E};
...
static uint64_t revs_quads(uint64_t bits) {
    uint64_t result = 0;
    for (int i = 0; i < 16; i++) {
...
static void fill_buffer(uint8_t bit) {
...
static void construct_EM410x_emulate(uint64_t id) {
    ...
    for (i = 0; i < 10; i++) {
        ...
        for (j = 0; j < 4; j++)
            fill_buffer(binary[j]);

        fill_buffer(binary[0] ^ binary[1] ^ binary[2] ^ binary[3]);
        for (j = 0; j < 4; j++)
            parity[j] ^= binary[j];
    }

    for (j = 0; j < 4; j++)
        fill_buffer(parity[j]);
...
void RunMod(void) {
    ...
    for (;;) {
        ...
        SpinUp(100);
        LED_Slot(selected);
        construct_EM410x_emulate(revs_quads(em4100emul_low[selected]));

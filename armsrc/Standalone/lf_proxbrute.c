...
#define STATE_BRUTE 1
    ...
    for (;;) {
        ...
        if (state == STATE_READ) {
            ...
            // findone, state_high, state_low
            lf_hid_watch(1, &state_high, &state_low, true);

            Dbprintf("[=]   recorded | %x%08x", state_high, state_low);

            // got nothing. blink and loop.
            if (state_high == 0 && state_low == 0) {
            ...
            /*
              ...
              Folstate_lowing code is a trivial brute forcer once you read a valid tag
            ...
            */
            ...
            Dbprintf("[=] simulating | %08x%08x", state_high, state_low);

            for (uint16_t i = state_low - 1; i > 0; i--) {
                ...
                Dbprintf("[=] trying Facility = %08x ID %08x", state_high, i);

                // state_high, i, ledcontrol,  timelimit 20000
                CmdHIDsimTAGEx(0, state_high, i, 0, false, 20000);

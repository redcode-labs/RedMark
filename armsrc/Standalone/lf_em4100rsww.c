...
static void SaveIDtoFlash(uint64_t id) {
    ...
    if (exists_in_spiffs(ffilename))
        rdv40_spiffs_append(ffilename, &bt[0], 5, RDV40_SPIFFS_SAFETY_NORMAL);
    else
        rdv40_spiffs_write(ffilename, &bt[0], 5, RDV40_SPIFFS_SAFETY_NORMAL);

...
static int CmdShowDict(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "mem dict",
                  "Show keycount of dictionaries loaded to flash",
                  "mem dict"
                 );
    ...
    CLIParserFree(ctx);
    clearCommandBuffer();
    SendCommandNG(CMD_MEM_DICT, NULL, 0);
    return PM3_SUCCESS;
}
...
static int CmdFlashMemInfo(const char *Cmd) {
    ...
    bool verbose = true;//arg_get_lit(ctx, 4);
    bool shall_write = false;
    shall_write = arg_get_lit(ctx, 4);
    CLIParserFree(ctx);
...
static command_t CommandTable[] = {
    ...
    {"dict",     CmdShowDict,    IfPm3Flash,  "Show dictionaries loaded to flashmem"},

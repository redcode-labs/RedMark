...
static int CmdLFTune(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_int0("t", "timeout", "<n>", "Exit tuning after <n> seconds"),
        arg_lit0(NULL, "max", "Use max ADC treshold for tuning"),
        arg_param_end
    };
    ...
    int8_t mxt = arg_get_int_def(ctx, 7, 10);
    int8_t adc_max = arg_get_lit(ctx, 8);
    CLIParserFree(ctx);
...
int CmdLFConfig(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "lf conf",
                  ...
                  "lf conf                    --> shows current config\n"
                  "lf conf -b 8 --125         --> samples at 125 kHz, 8 bps\n"
                  "lf conf -b 4 --134 --dec 3 --> samples at 134 kHz, averages three samples into one, stored with a resolution of 4 bits per sample\n"
                  "lf conf --trig 20 -s 10000 --> trigger sampling when above 20, skip 10 000 first samples after triggered\n"
                  "lf conf --reset            --> reset back to default values\n"
                 );
...
int CmdLFRead(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("l", "longer", "show plot window for longer time"),
        arg_lit0("n", "noplot", "do not show plot window"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    bool longer = arg_get_lit(ctx, 4);
    bool no_graph = arg_get_lit(ctx, 5);
    CLIParserFree(ctx);

    int hide_plot_after = 1400;
    if(longer){
        hide_plot_after = 15000;
    }
    if (g_session.pm3_present == false)
        return PM3_ENOTTY;
...
static bool CheckChipType(bool getDeviceData) {
    ...
    if (tryDetectP1(true)) {
        ...
        PrintAndLogEx(HINT, "Hint: try " _YELLOW_("`lf 5x`") " commands");
        retval = true;
        goto out;
    }
...
int CmdLFfind(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("n", NULL, "Do not search for unknown tags."),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    bool use_gb = arg_get_lit(ctx, 1);
    bool search_cont = arg_get_lit(ctx, 2);
    bool no_search_unk = arg_get_lit(ctx, 3);
    CLIParserFree(ctx);
    int found = 0;
    ...
    */
    ...
    if (! no_search_unk) {
        ...
        bool ghintsstate = g_session.show_hints;
        ...
        g_session.show_hints = ghintsstate;
    }
...
static command_t CommandTable[] = {
    ...
    {"5x",          CmdLFT55XX,         AlwaysAvailable, "[ T55xx CHIPs...             ]"},
    {"viking",      CmdLFViking,        AlwaysAvailable, "[ Viking RFIDs...            ]"},
    {"visa",        CmdLFVisa2k,        AlwaysAvailable, "[ Visa2000 RFIDs...          ]"},
//    {"zx",          CmdLFZx8211,        AlwaysAvailable, "{ ZX8211 RFIDs...            }"},
    {"-----------", CmdHelp,            AlwaysAvailable, "--------------------- " _CYAN_("General") " ---------------------"},
    {"conf",        CmdLFConfig,        IfPm3Lf,         "Get/Set config for LF sampling, bit/sample, decimation, frequency"},

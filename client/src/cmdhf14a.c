...
/*static int CmdHF14AData(const char *Cmd) {
    bool retry_select = false;
    int select_delay = 2500;
    int ack_timeout = 4000;

    CLIParserContext *ctx;
    CLIParserInit(&ctx, "hf 14a data",
                  "Collects basic info about tag (UID + SAK + ATQA + ATS)",
                  "hf 14a data\n");
    ...
    //memcpy(&card, (iso14a_card_select_t *)resp.data.asBytes, sizeof(iso14a_card_select_t));
    Hf14443_4aGetCardData((iso14a_card_select_t *)resp.data.asBytes);
    return PM3_SUCCESS;
}*/
...
int CmdHF14AReader(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_int0("n", "num", "<dec>", "Exit after reading <num> cards"),
        arg_lit0("p", "plot", "Show plot before exit"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    int numreads = arg_get_int(ctx, 8);
    bool show_plot = arg_get_lit(ctx, 9);
    CLIParserFree(ctx);
...
static int CmdHF14AInfo(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "hf 14a info",
                  ...
                  "hf 14a info -v -> shows full information about the card\n");

    void *argtable[] = {
        arg_param_begin,
        arg_lit0("v",  "verbose",   "adds some information to results"),
        arg_lit0("n",  "nonacktest",   "disable test for nack bug"),
        arg_lit0("s",  "noaidsearch", "disable check if AIDs from aidlist.json is present on the card and prints information about found AIDs"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
static int CmdHF14AChaining(const char *Cmd) {
    ...
    if (len && (!strcmp(str->sval[0], "enable") || !strcmp(str->sval[0], "1")))
        g_apdu_in_framing_enable = true;

    if (len && (!strcmp(str->sval[0], "disable") || !strcmp(str->sval[0], "0")))
        g_apdu_in_framing_enable = false;
...
int infoHF14A(bool verbose, bool do_nack_test, bool do_aid_search) {
    ...
    */
    ...
    if (isMifareClassic) {
        ...
        if (res == NONCE_STATIC)
            PrintAndLogEx(SUCCESS, "Static nonce: " _GREEN_("[+]"));

        if (res == NONCE_FAIL)
            PrintAndLogEx(SUCCESS, "Static nonce:  " _RED_("[x]"));
        ...
        if (res == NONCE_NORMAL) {
            ...
            if (res == 1)
                PrintAndLogEx(SUCCESS, _CYAN_("PRNG: ") _GREEN_("[+]"));
            else if (res == 0)
                PrintAndLogEx(SUCCESS, _CYAN_("PRNG: ") _RED_("[x]"));
            else
                PrintAndLogEx(FAILED, _CYAN_("PRNG:  ") _YELLOW_("[?]"));
...
static command_t CommandTable[] = {
    ...
    //{"data",        CmdHF14AData,         IfPm3Iso14443a,  "Obtain basic data about the tag"},

...
static int CmdEM410xBrute(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_u64_0(NULL, "clk", "<dec>", "<32|64> clock (default 64)"),
        arg_u64_0(NULL, "delay", "<dec>", "pause delay in milliseconds between UIDs simulation (default: 800ms)"),
    ...
    uint32_t delay = arg_get_u32_def(ctx, 2, 800);
...
static int CmdEM410xClone(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0(NULL, "a", "try all clock rates"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    ...
    int uid_len = 0;
    uint8_t uid[5] = {0};
    uint8_t clrates[4] = {16,32,40,64};
    CLIGetHexWithReturn(ctx, 2, uid, &uid_len);
    bool q5 = arg_get_lit(ctx, 3);
    bool em = arg_get_lit(ctx, 4);
    bool all = arg_get_lit(ctx, 5);
    CLIParserFree(ctx);
    if (! (q5 || em)){
        q5 = true;
    }
    ...
    if (all){
        for (uint32_t c = 0; c < sizeof(clrates); ++c) {
            uint8_t clock = clrates[c];
            payload.clock = clock;
            clearCommandBuffer();

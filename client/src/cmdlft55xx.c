...
int printConfiguration(t55xx_conf_block_t b) {
    PrintAndLogEx(SUCCESS, " Chip type......... " _GREEN_("%s"), (b.Q5) ? "Q5/T5555" : "T55x7");
    PrintAndLogEx(SUCCESS, " Password set...... %s", (b.usepwd) ? _RED_("Yes") : _GREEN_("No"));
    PrintAndLogEx(SUCCESS, " Block0............ %08X %s", b.block0, GetConfigBlock0Source(b.block0Status));
    PrintAndLogEx(SUCCESS, " Seq. terminator... %s", (b.ST) ? _GREEN_("Yes") : _RED_("No"));
    PrintAndLogEx(SUCCESS, " Modulation........ " _GREEN_("%s"), GetSelectedModulationStr(b.modulation));
    PrintAndLogEx(SUCCESS, " Bit rate.......... %s", GetBitRateStr(b.bitrate, (b.block0 & T55x7_X_MODE && (b.block0 >> 28 == 6 || b.block0 >> 28 == 9))));
    PrintAndLogEx(SUCCESS, " Inverted.......... %s", (b.inverted) ? _GREEN_("Yes") : "No");
    PrintAndLogEx(SUCCESS, " Offset............ %d", b.offset);
    PrintAndLogEx(SUCCESS, " Downlink mode..... %s", GetDownlinkModeStr(b.downlink_mode));
    if (b.usepwd) {
...
static int CmdT55xxBruteForce(const char *Cmd) {
    ...
    void *argtable[3 + 6] = {
        ...
        arg_lit0("n", "noaq", "skip aquisition phase for faster bruteforce")
    };
    uint8_t idx = 3;
    arg_add_t55xx_downloadlink(argtable, &idx, T55XX_DLMODE_ALL, T55XX_DLMODE_ALL);
    CLIExecWithReturn(ctx, Cmd, argtable, true);
...
//    else
    ...
    while (found == 0) {
        ...
        if(no_aq){
            found = t55xx_try_one_password_no_aq(curr, downlink_mode, ra);
        } else {
...
uint8_t t55xx_try_one_password_no_aq(uint32_t password, uint8_t downlink_mode,  bool try_all_dl_modes) {
...
static command_t CommandTable[] = {
    ...
    {"clh",    CmdT55xxCloneHelp,       IfPm3Lf,         "Shows the available clone commands"},
    ...
    {"rstread",    CmdResetRead,            IfPm3Lf,         "Send Reset Cmd then lf read the stream to attempt to identify the start of it"},
    ...
    {"brt",          CmdT55xxBruteForce,      IfPm3Lf,         "Simple bruteforce attack to find password"},

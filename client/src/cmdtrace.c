...
static uint8_t desfire_uid[10] = {0};
static uint8_t extract_sak[1] = {0};
static uint8_t extract_bcc[1] = {0};
...
static uint16_t extractChall_ev2(uint16_t tracepos, uint8_t *trace, uint8_t cmdpos, uint8_t long_jmp) {
    ...
    PrintAndLogEx(INFO, "EV2 ---> %s " NOLF, sprint_hex_inrow(next_hdr->frame + 1, 16));
...
static uint16_t extractChallenges(uint16_t tracepos, uint16_t traceLen, uint8_t *trace) {
    ...
    if (tracepos + TRACELOG_HDR_LEN + data_len + TRACELOG_PARITY_LEN(hdr) > traceLen) {
        PrintAndLogEx(INFO, "trace pos offset %"PRIu64 " larger than reported tracelen %u",
    ...
    if (data_len <= 6){
        switch (frame[0]) {
            case 0x26:{
                if (data_len == 1){
                    PrintAndLogEx(INFO, "REQA");
                    break;
                }
            }
            case 0x05:{
                if (data_len == 1){
                    PrintAndLogEx(INFO, "REQB");
                    break;
                }
            }
            case 0xc0:{
                PrintAndLogEx(INFO, "Decrement of block %X", frame[1]);
                break;
            }
            case 0xc1:{
                PrintAndLogEx(INFO, "Increment of block %X", frame[1]);
                break;
            }
            case 0xc2:{
                PrintAndLogEx(INFO, "Restore of block %X", frame[1]);
                break;
            }
                break;
            case 0xb0:{
                PrintAndLogEx(INFO, "Transfer of block %X", frame[1]);
                break;
            }
            case 0x50:{
                if (frame[1] == 0x00){
                    PrintAndLogEx(WARNING, "HALT");
                }
                break;
            }
            case 0x30:{
                if (data_len > 3){
                    PrintAndLogEx(INFO, "Read block [%X] with key A", frame[1]);
                }
                break;
            }
            case 0x31:{
                if (data_len > 3){
                    PrintAndLogEx(INFO, "Read block [%X] with key B", frame[1]);
                }
                break;
            }
            case MIFARE_AUTH_KEYA: {
                if (data_len > 3) {
                    PrintAndLogEx(INFO, "Auth attempt to block [%X] with key A", frame[1]);
                }
                break;
            }
            case MIFARE_AUTH_KEYB: {
                if (data_len > 3) {
                    PrintAndLogEx(INFO, "Auth attempt to block [%X] with key B", frame[1]);
                }
                break;
            }
        }
    ...
    switch (frame[0]) {
        case MIFARE_ULC_AUTH_1: {
            ...
            PrintAndLogEx(INFO, "MFU-C 3DES ---> " _LIGHT_GREEN_("%s") NOLF, sprint_hex_inrow(next_hdr->frame + 1, 8));
    ...
    switch (frame[0]) {
        case ISO14443A_CMD_ANTICOLL_OR_SELECT: {
            // 93 20 = Anticollision (usage: 9320 - answer: 4bytes UID+BCC)
            ...
            if (frame[1] == 0x70) {
                if (frame[2] == 0x88) {
                    PrintAndLogEx(SUCCESS, "Chinese access control system detected");
                    memcpy(extract_uid, frame + 3, 3);
                    memcpy(extract_bcc, frame + 6, 1);
                    extract_uidlen = 3;
                    PrintAndLogEx(INFO, "UID -?-> " _MAGENTA_("%s"), sprint_hex_inrow(extract_uid, extract_uidlen));
                    PrintAndLogEx(INFO, "BCC -?-> " _CYAN_("%s"), sprint_hex_inrow(extract_bcc, 1));
                } else {
                ...
                if (hdr->isResponse){
                    if (extract_uidlen != 3){
                        memcpy(extract_sak, frame + 3, 1);
                        PrintAndLogEx(INFO, "SAK ---> " _GREEN_("[%s]"), sprint_hex_inrow(extract_sak, 1));
                    }
                } else {
                    if (extract_uidlen != 3){
                        memcpy(extract_sak, frame, 1);
                        PrintAndLogEx(INFO, "SAK ---> " _MAGENTA_("[%s]"), sprint_hex_inrow(extract_sak, 1));
                    }
                }
            }
            break;
    ...
    //    return tracepos;
    //}
    PrintAndLogEx(INFO, "DESFire detected");
    ...
    for (uint8_t i = 0; i < 2; i++, pos++) {
        ...
        if (data_len == 16){
            int key_part_2_idx = 8;
            int parts_equal = 0;
            for (int x = 0; x < key_part_2_idx; x++){
                if (frame[x] == frame[key_part_2_idx+x]){
                    parts_equal += 1;
                }
            }
            if (parts_equal == 8){
                PrintAndLogEx(INFO, "1kDES key ---> " _LIGHT_MAGENTA_("%s"), sprint_hex_inrow(frame, 8));
                //tracelog_hdr_t *next_hdr = (tracelog_hdr_t *)(trace + tracepos);
                //tracepos += SKIP_TO_NEXT(next_hdr);
                return tracepos;
            }
        ...
        switch (frame[pos]) {
            ...
            // Get file ID for PICC
            case 0x6f: {
                tracelog_hdr_t *next_hdr = (tracelog_hdr_t *)(trace + tracepos);
                tracepos += SKIP_TO_NEXT(next_hdr);
                if (next_hdr->frame[0] == 0x00){
                    PrintAndLogEx(SUCCESS, "Get file IDs for PICC --->", _LIGHT_GREEN_("[OK]"));
                } else {
                    PrintAndLogEx(ERR, "Get file IDs for PICC --->", _LIGHT_RED_("[ERR]"));
                }
                return tracepos;
            }
            ...
            case 0x6a: {
                ...
                tracepos += SKIP_TO_NEXT(next_hdr);
                if (next_hdr->frame[0] == 0x00){
                    PrintAndLogEx(SUCCESS, _GREEN_("Blank DESFire card detected"));
                }
                return tracepos;
            }
            ...
            case 0x60: {
                tracelog_hdr_t *next_hdr = (tracelog_hdr_t *)(trace + tracepos);
                tracepos += SKIP_TO_NEXT(next_hdr);
                while (next_hdr->frame[0] == 0xaf){
                    tracepos += SKIP_TO_NEXT(next_hdr);
                }
                if (next_hdr->frame[0] == 0x00){
                    memcpy(desfire_uid, next_hdr->frame + 1, 7);
                    PrintAndLogEx(INFO, "UID ---> " _MAGENTA_("%s"), sprint_hex_inrow(desfire_uid, 7));
                } else if (next_hdr->frame[0] == 0x9d){
                    PrintAndLogEx(ERR, _RED_("GetVersion authentication error"));
                }
                return tracepos;
            }
            ...
            case 0x64 : {
                if (data_len == 2){
                    if (frame[pos+1] == 0x00){
                        //uint16_t tmp_tracepos = tracepos;
                        tracelog_hdr_t *next_hdr = (tracelog_hdr_t *)(trace + tracepos);
                        tracepos += SKIP_TO_NEXT(next_hdr);
                        if ((next_hdr->frame[0] == 0x00 && next_hdr->frame[1] == 0x00)){
                            PrintAndLogEx(SUCCESS, "Get version for key 00 --->", _LIGHT_GREEN_("[OK]"));
                        } else {
                            PrintAndLogEx(ERR, "Get version for key 00 --->", _LIGHT_RED_("[ERR]"));
                        }
                    }
                }
                return tracepos;
            ...
            case 0x5a : {
                if (frame[pos+1] == 0x00){
                    ...
                    tracelog_hdr_t *next_hdr = (tracelog_hdr_t *)(trace + tracepos);
                    tracepos += SKIP_TO_NEXT(next_hdr);
                    if ((next_hdr->frame[0] == 0x00 && next_hdr->data_len == 1)){
                        PrintAndLogEx(SUCCESS, "PICC select OK");
                    }
                    //tracepos = tmp_tracepos;
                    return tracepos;
                }
                return tracepos;
            }
            ...
            case 0x45: {
                ...
                tracepos += SKIP_TO_NEXT(next_hdr);
                if (next_hdr->frame[0] != 0x00){
                    PrintAndLogEx(ERR, "Permission denied for key settings retrieval");
                }
                if (next_hdr->frame[1] == 0x0f){
                    PrintAndLogEx(SUCCESS, "Master key is changeable");
                }
                if (next_hdr->frame[2] != 0x00){
                    PrintAndLogEx(SUCCESS, "Currently configured keys: 0x%s (%d)", sprint_hex_inrow(next_hdr->frame+2, 1), next_hdr->frame[2]);
                }
                //tracepos = tmp_tracepos;
                return tracepos;
            }
            case MFDES_AUTHENTICATE: {
                if(frame[pos+1] == 0x00){
                    PrintAndLogEx(INFO, "PICC authentication with key 0x00");
                }
                if(frame[pos+1] == 0x01){
                    PrintAndLogEx(INFO, "PICC authentication with key 0x01");
                }
...
static int CmdTraceExtract(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("1", "buffer", "do not use data from trace buffer"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    bool use_bfr = arg_get_lit(ctx, 1);
    ...
    if (gs_traceLen == 0){
        PrintAndLogEx(SUCCESS, "Empty trace (len = " _RED_("%u") " bytes)", gs_traceLen);
        return PM3_SUCCESS;
    } else {
        PrintAndLogEx(SUCCESS, "Recorded trace (len = " _LIGHT_BLUE_("%u") " bytes)", gs_traceLen);
    }
...
int CmdTraceList(const char *Cmd) {
    ...
    CLIParserInit(&ctx, "trace list",
                  ...
                  "\n"
                  "trace list -t 14a      -> interpret as " _LIGHT_GREEN_("ISO14443-A") "\n"
                  "trace list -t 14b      -> interpret as " _LIGHT_GREEN_("ISO14443-B") "\n"
                  "trace list -t 15       -> interpret as " _LIGHT_GREEN_("ISO15693") "\n"
                  "trace list -t 7816     -> interpret as " _LIGHT_GREEN_("ISO7816-4") "\n"
                  "trace list -t cryptorf -> interpret as " _LIGHT_GREEN_("CryptoRF") "\n\n"
                  "trace list -t des      -> interpret as " _LIGHT_GREEN_("MIFARE DESFire") "\n"
                  "trace list -t felica   -> interpret as " _LIGHT_GREEN_("ISO18092 / FeliCa") "\n"
                  "trace list -t hitag1   -> interpret as " _LIGHT_GREEN_("Hitag1") "\n"
                  "trace list -t hitag2   -> interpret as " _LIGHT_GREEN_("Hitag2") "\n"
                  "trace list -t hitags   -> interpret as " _LIGHT_GREEN_("HitagS") "\n"
                  "trace list -t iclass   -> interpret as " _LIGHT_GREEN_("iCLASS") "\n"
                  "trace list -t legic    -> interpret as " _LIGHT_GREEN_("LEGIC") "\n"
                  "trace list -t lto      -> interpret as " _LIGHT_GREEN_("LTO-CM") "\n"
                  "trace list -t mf       -> interpret as " _LIGHT_GREEN_("MIFARE Classic") " and decrypt crypto1 stream\n"
                  "trace list -t seos     -> interpret as " _LIGHT_GREEN_("SEOS") "\n"
                  "trace list -t thinfilm -> interpret as " _LIGHT_GREEN_("Thinfilm") "\n"
                  "trace list -t topaz    -> interpret as " _LIGHT_GREEN_("Topaz") "\n"
                  "\n"
    ...
    void *argtable[] = {
        ...
        arg_str0("f", "dict", "<fn>", "use dictionary keys file"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    ...
    if (show_hex)
        PrintAndLogEx(INFO, "syntax to use: " _YELLOW_("`text2pcap -t \"%%S.\" -l 264 -n <input-text-file> <output-pcapng-file>`"));
...
static command_t CommandTable[] = {
    ...
    {"x", CmdTraceExtract,  AlwaysAvailable, "Extract authentication challenges found in trace"},
    {"l",    CmdTraceList,     AlwaysAvailable, "List protocol data in trace buffer"},
    {"v",    CmdTraceLoad,     AlwaysAvailable, "Load trace from file"},
    {"s",    CmdTraceSave,     AlwaysAvailable, "Save trace buffer to file"},
    {NULL, NULL, NULL, NULL}
};

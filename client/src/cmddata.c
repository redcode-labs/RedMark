...
int CmdDecimate(const char *Cmd) {
...
static int CmdBar(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "d bar",
                  "Toggle bar slider",
                  "d bar -0"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("0", NULL, "hide slider"),
        arg_lit0("1", NULL, "show slider"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    bool hide = arg_get_lit(ctx, 1);
    bool show = arg_get_lit(ctx, 2);
    CLIParserFree(ctx);

    if(hide && show){
        PrintAndLogEx(FAILED, "Set either flag");
        return PM3_EINVARG;
    }
    if(show){
...
int CmdMod(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "d mod",
                  "Toggle modulator",
                  "d mod -0"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_lit0("0", NULL, "hide modulator on plot window"),
        arg_lit0("1", NULL, "show modulator on plot window"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    bool hide = arg_get_lit(ctx, 1);
    ...
    if(show){
        setClockGrid(0, 0);
        g_DemodBufferLen = MAX_DEMOD_BUF_LEN;
        RepaintGraphWindow();
    } else if(hide){
        setClockGrid(0, 0);
        g_DemodBufferLen = 0;
...
static int CmdSetGraphMarkers(const char *Cmd) {
    ...
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    g_CursorCPos = arg_get_u32_def(ctx, 1, g_CursorCPos);
    g_CursorDPos = arg_get_u32_def(ctx, 2, g_CursorDPos);
    CLIParserFree(ctx);
...
static int CmdHexsamples(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "data hex",
                  "Dump big buffer as hex bytes",
                  "data hex -n 128  -->  dumps 128 bytes from offset 0"
                 );
    void *argtable[] = {
        arg_param_begin,
...
int CmdTuneSamples(const char *Cmd) {
    ...
    void *argtable[] = {
        arg_param_begin,
        arg_int0("t", "timeout", "<n>", "Max timeout of tuning (default: 20)"),
        arg_lit0("f", "fast", "Perform fast tuning"),
        arg_lit0("v", "volt", "Set max voltage for usability check"),
        arg_lit0("d", "demod", "Hide demodulator plot line"),
        arg_lit0("g", "graph", "Show recorded LF plot"),
        arg_lit0("L", "low", "Tune only LF antenna"),
        //arg_lit0("i", "inc", "Use increased HF usability voltage treshold to avoid false negatives"),
        arg_lit0("s", "smol", "Use smaller voltage treshold for antenna verdict"),
        arg_lit0("n", "norst", "Do not change position of C and D markers"),
        arg_lit0("b", "nobuff", "Do not overwrite the plot with values obtained from tuning"),
        arg_lit0("H", "high", "Tune only HF"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);
    int tout = arg_get_int_def(ctx, 1, 0);
    bool fast = arg_get_lit(ctx, 2);
    bool volt = arg_get_lit(ctx, 3);
    bool demod = arg_get_lit(ctx, 4);
    bool graph = arg_get_lit(ctx, 5);
    bool lf_only = arg_get_lit(ctx, 6);
    bool smol_volts = arg_get_lit(ctx, 7);
    bool norst = arg_get_lit(ctx, 8);
    bool nobuff = arg_get_lit(ctx, 9);
    bool hfonly = arg_get_lit(ctx, 10);
    CLIParserFree(ctx);

    int timeout = 0;
    int timeout_max = 20;
    uint16_t test1 = 0;
    int hf_unusable_volts = 3000;//2000;
    int lf_unusable_volts = 2000;//1800;
    if(tout){
        timeout_max = tout;
    }
    if(fast){
    ...
    if(volt){
        hf_unusable_volts = 3000;
        hf_unusable_volts = 2000;
    }

    if(smol_volts){
        hf_unusable_volts = 240;
        lf_unusable_volts = 1600;
    }
    ...
    PrintAndLogEx(INFO, "---------- " _MAGENTA_("TUNE STARTED") " ------------------------");
    ...
    // Empirical measures in mV (3% margin)
    tune_hf:
    double vdd_rdv4 = 9000;
    double vdd_other = 5400;
    if(smol_volts){
        vdd_rdv4 = 8700;
        vdd_other = 5100;
        //vdd_rdv4 = 5000;
    ...
    if (package->peak_v > NON_VOLTAGE && package->peak_f > 0) {
        ...
        //if (lfq1 > 3) {
            ...
            /*if ((approx_vdd > approx_vdd_other_max * 1.01) && (!IfPm3Rdv4Fw())) {
                PrintAndLogEx(WARNING, "Contradicting measures seem to indicate you're running a " _RED_("PM3GENERIC firmware on a RDV4"));
            ...
            if ((approx_vdd < approx_vdd_other_max * 0.99) && (IfPm3Rdv4Fw())) {
                PrintAndLogEx(WARNING, "Contradicting measures seem to indicate you're running a " _RED_("PM3_RDV4 firmware on a generic device"));
                PrintAndLogEx(WARNING, "False positives is possible but please check your setup");
            }*/
        //}
    }
    ...
    // LF evaluation
    if (package->peak_v < lf_unusable_volts)
        snprintf(judgement, sizeof(judgement), _RED_(" [x] UNUSABLE"));
    ...
    PrintAndLogEx((package->peak_v < lf_unusable_volts) ? WARNING : SUCCESS, "LF antenna is %s", judgement);
    ...
    if(!lf_only){
        ...
        if (package->v_hf >= hf_unusable_volts) {
            // Q measure with Vlr=Q*(2*Vdd/pi)
        ...
        PrintAndLogEx((package->v_hf < hf_unusable_volts) ? WARNING : SUCCESS, "HF antenna is %s (%d mV)", judgement, package->v_hf);
...
int CmdLtrim(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "d l",
                  "Trim samples from left of trace",
                  "d l -i 300   --> keep 300 - end"
                 );
    void *argtable[] = {
        arg_param_begin,
...
static int CmdRtrim(const char *Cmd) {
    ...
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "d r",
                  "Trim samples from right of trace",
                  "d r -i 4000    --> keep 0 - 4000"
                 );
    void *argtable[] = {
        arg_param_begin,
...
int CmdXtrim(const char *Cmd) {

    CLIParserContext *ctx;
    CLIParserInit(&ctx, "d x",
                  "Trim samples around a cursor",
                  "d x -i 1000    --> extract 1000 samples from before and after cursor"
                 );
    void *argtable[] = {
        arg_param_begin,
        arg_u64_1("i", "idx", "<dec>", "number of samples to trim"),
        arg_lit0("b", NULL, "use cursor B instead"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, false);
    uint32_t ds = arg_get_u32(ctx, 1);
    ...
    int cursor = g_CursorCPos;
    if(use_b){
        cursor = g_CursorDPos;
    }

    int trim_start = cursor - ds;
    int trim_stop = cursor + ds;

    for (uint32_t i = trim_stop; i < g_GraphTraceLen; ++i)
        g_GraphBuffer[i - trim_stop] = g_GraphBuffer[i];

    g_GraphTraceLen -= trim_stop;
    g_DemodStartIdx -= trim_stop;
    RepaintGraphWindow();

    g_GraphTraceLen = trim_start;
    RepaintGraphWindow();
    return PM3_SUCCESS;
}
...
int try_detect_modulation(void) {
...
static command_t CommandTable[] = {
    ...
    {"g",               CmdGrid,                 AlwaysAvailable,  "overlay grid on graph window"},
    {"l",               CmdLtrim,                AlwaysAvailable,  "Trim samples from left of trace"},
    {"m",               CmdMtrim,                AlwaysAvailable,  "Trim out samples from the specified start to the specified stop"},
    {"x",               CmdXtrim,                AlwaysAvailable,  "Trim samples around a marker"},
    {"norm",            CmdNorm,                 AlwaysAvailable,  "Normalize max/min to +/-128"},
    {"p",               CmdPlot,                 AlwaysAvailable,  "Show graph window"},
    {"r",               CmdRtrim,                AlwaysAvailable,  "Trim samples from right of trace"},
    {"bar",             CmdBar,                  AlwaysAvailable,  "Toggle slider bar"},
    {"mod",             CmdMod,                  AlwaysAvailable,  "Toggle demodulator on plot"},
    {"sm",              CmdSetGraphMarkers,      AlwaysAvailable,  "Set blue and orange marker in graph window"},
    ...
    {"hex",             CmdHexsamples,           IfPm3Present,     "Dump big buffer as hex bytes"},
    ...
    {"dbg",             CmdSetDebugMode,         AlwaysAvailable,  "Set Debugging Level on client side"},

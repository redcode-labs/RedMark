...
int CmdHFSearch(const char *Cmd) {
    ...
    void *argtable[] = {
        ...
        arg_lit0("c", "cont", "do not exit on first match (continuous mode)"),
        arg_int0("n", "num", "<dec>", "exit after this amount of tags is detected (default: 2)"),
        arg_lit0("i", "iso", "search only for ISO tags"),
        arg_lit0("t", "tests", "perform more tests on found tags"),
        arg_param_end
    };
    CLIExecWithReturn(ctx, Cmd, argtable, true);

    bool verbose = arg_get_lit(ctx, 1);
    bool cont = arg_get_lit(ctx, 2);
    int maxtags = arg_get_int_def(ctx, 3, 1);
    bool isonly = arg_get_lit(ctx, 4);
    bool tests = arg_get_lit(ctx, 5);
    int found_tags = 0;
    bool hint_state = g_session.show_hints;
    ...
    if (isonly){
        ...
        if (IfPm3Iso14443a()) {
            int sel_state = infoHF14A(verbose, false, tests);
            if (sel_state > 0) {
        ...
        if (IfPm3Iso14443b()) {
            if (readHF14B(false, verbose) == PM3_SUCCESS) {
    ...
    if (IfPm3NfcBarcode()) {
        if (infoThinFilm(verbose) == PM3_SUCCESS) {
    ...
    if (IfPm3Iso14443a()) {
        if (reader_lto(false, verbose) == PM3_SUCCESS) {
    ...
    PrintAndLogEx(INPLACE, " Searching for ISO14443-A tag...");
    if (IfPm3Iso14443a()) {
        int sel_state = infoHF14A(verbose, tests, tests);
        if (sel_state > 0) {
            PrintAndLogEx(SUCCESS, "\nValid " _GREEN_("ISO 14443-A tag") " found\n");
            res = PM3_SUCCESS;
            found_tags++;
    ...
    if (IfPm3Iclass()) {
        if (read_iclass_csn(false, verbose) == PM3_SUCCESS) {
    ...
    if (IfPm3Legicrf()) {
        if (readLegicUid(false, verbose) == PM3_SUCCESS) {
    ...
    if (IfPm3Iso14443a()) {
        if (readTopazUid(false, verbose) == PM3_SUCCESS) {
    ...
    if (IfPm3Felica()) {
        if (read_felica_uid(false, verbose) == PM3_SUCCESS) {
    ...
    if (read_texkom_uid(false, verbose) == PM3_SUCCESS) {
    ...
    if (IfPm3Iso14443b()) {
        if (read_xerox_uid(false, verbose) == PM3_SUCCESS) {
    ...
    if (IfPm3Iso14443b()) {
        if (readHFCryptoRF(false, verbose) == PM3_SUCCESS) {
    ...
    res_check:
    if ((cont || found_tags < maxtags)){
        goto hf_search_start;
    }
    PROMPT_CLEARLINE;
    if (res != PM3_SUCCESS) {
        if (isonly){
            PrintAndLogEx(WARNING, _RED_("No known/supported ISO tags found"));
        }
        res = PM3_ESOFT;
    } else if (verbose) {
        PrintAndLogEx(SUCCESS, "Found %d tags", found_tags);
    }
    PrintAndLogEx(INFO, "Exiting from search . . .");
    g_session.show_hints = hint_state;
    DropField();
    return res;
}
...
static command_t CommandoTablo[] = {
...
//    {"cryptorf",    CmdHFCryptoRF,    AlwaysAvailable, "{ CryptoRF RFIDs...                   }"},
    ...
    {"tesla",       CmdHFTESL,        AlwaysAvailable, "{ TESLA Cards...                      }"},
...
int CmdHF(const char *Cmd) {
    clearCommandBuffer();
    return CmdsParse(CommandoTablo, Cmd);
}

int CmdHelp(const char *Cmd) {
    (void)Cmd; // Cmd is not used so far
    CmdsHelp(CommandoTablo);
    return PM3_SUCCESS;
}

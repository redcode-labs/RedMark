...
static int CmdScriptListCmd(const char *Cmd) {
    CLIParserContext *ctx;
    CLIParserInit(&ctx, "x c",
                  "List available Cmd scripts",
                  "x l"
                 );
    void *argtable[] = {
        arg_param_begin,
    ...
    CLIParserFree(ctx);
    PrintAndLogEx(SUCCESS, "\n" _MAGENTA_("< Cmd scripts >"));
    int ret = searchAndList(CMD_SCRIPTS_SUBDIR, ".cmd");
    if (ret != PM3_SUCCESS)
        return ret;
    return ret;
}
...
static command_t CommandTable[] = {
    ...
    {"l",  CmdScriptList,    AlwaysAvailable, "List available scripts"},
    {"c",  CmdScriptListCmd,    AlwaysAvailable, "List available Cmd scripts"},
    {"r",   CmdScriptRun,     AlwaysAvailable, "<name> - execute a script"},
    {NULL, NULL, NULL, NULL}
};

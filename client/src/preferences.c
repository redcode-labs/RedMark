...
int preferences_load(void) {
    ...
    // Set all defaults
    g_session.client_debug_level = 1;//cdbOFF;
    ...
    g_session.dense_output = true;//false;
...
static void showPlotSliderState(prefShowOpt_t opt) {
    if (g_session.overlay_sliders)
        ...
        PrintAndLogEx(INFO, "   %s show plot sliders...... "_RED_("off"), prefShowMsg(opt));
}
...
static void showBarModeState(prefShowOpt_t opt) {
    ...
    switch (g_session.bar_mode) {
        case STYLE_BAR:
            ...
            break;
        case STYLE_MIXED:
            PrintAndLogEx(INFO, "   %s barmode................ "_YELLOW_("mixed"), prefShowMsg(opt));
            break;
        case STYLE_VALUE:

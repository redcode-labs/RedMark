...
pm3_device_t *pm3_open(const char *port) {
    pm3_init();
    OpenProxmark(&g_session.current_device, port, false, 60, false, USART_BAUD_RATE);
    if (g_session.pm3_present && (TestProxmark(g_session.current_device) != PM3_SUCCESS)) {
        PrintAndLogEx(ERR, _RED_("ERROR:") " command send failure\n");
    ...
    if (!g_session.pm3_present)
        PrintAndLogEx(INFO, "Running in " _RED_("OFFLINE") " mode");

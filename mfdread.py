...
def print_info(data):
    ...
    for c in range(0, len(blocksmatrix)):
        ...
        uid = bashcolors.MAGENTA + blocksmatrix[0][0][0:8] + bashcolors.ENDC 
        bcc = bashcolors.CYAN + blocksmatrix[0][0][8:10] + bashcolors.ENDC
        sak = bashcolors.YELLOW + blocksmatrix[0][0][10:12] + bashcolors.ENDC
        atqa = bashcolors.RED + blocksmatrix[0][0][12:16] + bashcolors.ENDC
        rest = blocksmatrix[0][0][16:32]

        blocksmatrix[c][sector_size] = keyA + accbits + keyB
        if c == 0:
            blocksmatrix[c][0] = uid + bcc + sak + atqa + rest

    print("[ %d bytes == %d sectors ]" % (len(data), sector_number))
    ...
    for q in range(0, len(blocksmatrix)):
        ...
        for z in range(0, len(blocksmatrix[q])):
            ...
            if isinstance(blockrights[q][z], BitArray):
                ...
                accbits = bashcolors.RED + "ERR" + bashcolors.ENDC
